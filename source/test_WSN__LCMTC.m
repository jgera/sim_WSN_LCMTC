...
% sono circa:
PacchettiNonTX=round(PacchettiTX-PacchettiSuccTX);
...
for i=1:WSN_node_number;
    nodo_WSN(i)=WSN_node(WSN_type,WSNnode_daily_tx(i),resolution,simulation_length);    % il numero di trasmissioni giornaliere del nodo è random, fra 1 e "max_WSN_daily_tx"
...
LCMTC_BatteryLevel=0.5;     % the level of the concentrator's battery (50% of max capacity)

%creo un oggetto "concentratore_LCMTC"
concentratore_LCMTC=LCMTC_node(LCMTC_type,LCMTC_daily_tx,resolution,simulation_length,sequenzadiTXTotale,LCMTC_BatteryLevel,LTEIAT_mean);
